...
async function sendImage(base64: string, accessToken: string, setLoading: Dispatch<SetStateAction<boolean>>) {
    const { code, json } = await camera("ai/readimage", base64, accessToken, setLoading);
    if (code === 200) {
        console.log(json);
    }
    return;
            ...
            {/* {
                enableWebcam ? (
                    ...
                    : <>
                        ...
                        />
                        <div className="flex flex-col gap-3 max-w-48 m-auto mt-5">
                            <button className="rounded-xl bg-black text-white p-1" onClick={() => setLoading((prev) => !prev)}>Ladda upp foto</button>
